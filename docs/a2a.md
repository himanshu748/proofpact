...
The connection panel's **Test connection** button sends a real read-only A2A message and displays the returned task. Demo identity remains synthetic and switchable. Production user accounts, independently verified participant identities and production authorization are still required before real customer use. Razorpay Route sandbox access is separately pending support; A2A cannot enable payments or bypass that prerequisite.
